...
async function loadDeliveryShipments() {
  ...
  try {
    ...
    const response =
      await fetch(
        SUPABASE_URL +
        "/rest/v1/shipments" +
        "?select=id,serial_no,received_date,lr_number,quantity,amount,payment_type,delivery_date,accounts_date,remarks,parties(name),branches(name)" +
      ...
      html += `
        ...
        <tr>
          ...
          <td>
            ...
            <button
              type="button"
              class="primary-button delivery-action"
...
/* ---------------------------------------------------------
   LOAD WHEN DELIVERY PAGE IS OPENED
   --------------------------------------------------------- */
